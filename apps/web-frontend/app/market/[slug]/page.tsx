...
export default function BinaryMarketPage() {
  ...
  const marketStats = React.useMemo(() => {
    ...
    const totalVolume = marketData.platforms.reduce(
      (sum, p) => sum + p.volume24h,
      0
    );
    const totalLiquidity = marketData.platforms.reduce(
      (sum, p) => sum + p.liquidity,
      0
    );
    const avgYesPrice =
      marketData.platforms.reduce((sum, p) => sum + p.yesPrice, 0) /
      marketData.platforms.length;
    return {
      totalVolume,
      totalLiquidity,
      avgYesPrice,
      platformCount: marketData.platforms.length,
    };
  }, [marketData]);
  ...
  const currentMarketPositions = React.useMemo(() => {
    ...
    const matchString = marketData.question
      .split(" ")
      .slice(0, 3)
      .join(" ")
      .toLowerCase();
    ...
    return allPositions.filter((position) =>
      position.market.toLowerCase().includes(matchString)
    );
  }, [marketData]);
  ...
  const activePlatform = React.useMemo(() => {
    ...
    return selectedPlatformId
      ? (marketData.platforms.find((p) => p.id === selectedPlatformId) ??
  ...
  const sidebarRelatedMarkets: SidebarMarket[] = marketData.platforms.map(
    (p) => ({
      id: Number(p.id),
        ...
        <div className="pr-5 py-6 space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                <Badge
                  variant="outline"
                  className="border-green-500/50 text-green-500 uppercase"
                >
                  Live
                </Badge>
              </div>
          ...
          <div className="space-y-6">
            <MarketChart
              ...
              title={
                activePlatform
                  ? `${activePlatform.platform} - ${activePlatform.title}`
                  : "Aggregated Market Prices"
              }
              currentYesPrice={
                activePlatform
                  ? activePlatform.yesPrice
                  : marketStats.avgYesPrice
              }
              currentNoPrice={
                activePlatform
                  ? activePlatform.noPrice
                  : 1 - marketStats.avgYesPrice
              }
              timeframe={timeframe}
              onTimeframeChange={setTimeframe}
              platform={
                activePlatform ? activePlatform.platform : "All Platforms"
              }
            />
            <MarketComparison
              ...
              <Card className="border-foreground/10">
                ...
                <CardContent className="space-y-4">
                  {currentMarketPositions.map((position) => {
                    ...
                    const isProfitable = position.profit
                      ? position.profit > 0
                      : false;
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            >
                              <CategoryIcon
                                className={`w-4 h-4 mr-1.5 ${getCategoryColor(position.category)}`}
                              />
                              {position.category}
                            </Badge>
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            >
                              ...
                              {position.avgPrice.toFixed(1)}¢ avg.
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
