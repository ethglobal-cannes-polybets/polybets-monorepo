...
interface BettingFormViewProps {
  ...
  selectedMarkets: SidebarMarket[];
  setSelectedMarkets: React.Dispatch<React.SetStateAction<SidebarMarket[]>>;
...
const PositionSelector: React.FC<{
  ...
  yesPrice: number;
  noPrice: number;
}> = ({ outcome, setOutcome, yesPrice, noPrice }) => (
...
}> = ({ amount, setAmount, calc, outcome, hasSelectedMarkets }) => (
  <div className="space-y-2">
    ...
    {amount && hasSelectedMarkets && (
      <BetCalculationCard calc={calc} outcome={outcome} amount={amount} />
    )}
  </div>
);
...
const AutoArbitrageToggle: React.FC<{
  autoArbitrage: boolean;
  setAutoArbitrage: (v: boolean) => void;
}> = ({ autoArbitrage, setAutoArbitrage }) => (
...
const StrategySelector: React.FC<{
  strategy: "maximize-shares" | "maximize-privacy";
  setStrategy: (s: "maximize-shares" | "maximize-privacy") => void;
  ...
  <div className="space-y-2">
    ...
    <RadioGroup
      value={strategy}
      onValueChange={(v) =>
        setStrategy(v as "maximize-shares" | "maximize-privacy")
      }
    >
...
const MarketSelector: React.FC<{
  ...
  selectedMarkets: SidebarMarket[];
  onToggle: (market: SidebarMarket, checked: boolean) => void;
  outcome: "yes" | "no";
  averagePrice: number;
}> = ({ markets, selectedMarkets, onToggle, outcome, averagePrice }) => (
  <div className="space-y-2">
    <div className="flex items-center justify-between">
...
export const BettingFormView: React.FC<BettingFormViewProps> = (props) => {
  ...
  // Handlers ---------------------------------------------------------------
  const handleMarketToggle = (market: SidebarMarket, checked: boolean) => {
