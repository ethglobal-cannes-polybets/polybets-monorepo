...
def extract_shares_from_api_response(
    response_data: Dict[str, Any], collateral_amount: float
) -> int:
    """
    ...
    shares_bought = (
        response_data.get("sharesMinted")  # New primary field
        or response_data.get("shares")
        or response_data.get("sharesBought")
        or response_data.get("shares_bought")
        or response_data.get("sharesReceived")
        or response_data.get("shares_received")
        or response_data.get("amount")
        or response_data.get("quantity")
        or 0
    )
...
POLYBETS_CONTRACT_ADDRESS = (
    ...
    or "0xd474F5ec5e1E7B14E0639F388A42637129D068Be"
)
...
def record_proxied_bet_on_contract(
    bet_slip_id: int, successful_bet: BetResponse, shares_bought: int, option: int
) -> bool:
    """
    ...
    try:
        ...
        print(
            f"     Market: {successful_bet.market_id} on marketplace {marketplace_id}"
        )
        print(f"     Option: {'YES' if option == 0 else 'NO'}")
        print(
            f"     Amount: ${successful_bet.collateral_amount:.2f} → {shares_bought} shares"
        )
        ...
        print(
            f"     Bet details: Market {successful_bet.market_id}, Amount ${successful_bet.collateral_amount:.2f}"
        )
        return False
...
def handle_buy_flow(bet_slip_id: int, bet_slip_data):
    ...
    try:
        ...
        marketplace_ids = decode_bytes32_array(
            bet_slip_data[8]
        )  # marketplaceIds at index 8
        ...
        if isinstance(pool_configs, Exception):
            ...
            if isinstance(execution_result, Exception):
                ...
                for i, bet in enumerate(execution_result.successful_bets, 1):
                    print(
                        f"    {i}. ${bet.collateral_amount:.2f} → {bet.endpoint_name} (market {bet.market_id})"
                    )
                ...
                print(
                    f"  📝 Recording {len(execution_result.successful_bets)} successful bets on contract..."
                )

                successful_recordings = 0
                for bet in execution_result.successful_bets:
                    # Extract shares bought from API response
                    shares_bought = extract_shares_from_api_response(
                        bet.response_data, bet.collateral_amount
                    )

                    # Record the proxied bet on the contract
                    if record_proxied_bet_on_contract(
                        bet_slip_id, bet, shares_bought, option
                    ):
                        successful_recordings += 1

                print(
                    f"  ✅ Successfully recorded {successful_recordings}/{len(execution_result.successful_bets)} bets on contract"
                )

                failure_reason = (
                    "" if execution_result.success_rate > 0 else "All bets failed"
                )

        # Update Bet Slip Status
        update_bet_slip_status(
            bet_slip_id, pool_configs, execution_result, failure_reason
        )
    ...
    try:
        ...
        for bet_id in proxied_bet_ids:
            try:
                ...
                elif schema == "canibeton_variant3":
                    endpoint_name = "degen-execution-chamber"
                elif schema == "canibeton_variant4":
                    endpoint_name = "nihilistic-prophet-syndicate"
                else:
                ...
                if response.status_code == 200 or response.status_code == 201:
                    ...
                    if record_proxied_bet_sold(
                        bet_id, shares_to_sell, collateral_received
                    ):
                        successful_sales += 1
                    ...
                    print(
                        f"     ❌ Sell failed: HTTP {response.status_code}: {error_data}"
                    )

                    # Log slippage errors but don't retry with parameters that don't exist in the API
                    if (
                        "SlippageToleranceExceeded" in response.text
                        or "slippage" in response.text.lower()
                    ):
                        print(
                            f"     ⚠️  Slippage tolerance exceeded - this is handled by the API internally"
                        )
...
def record_proxied_bet_sold(
    bet_id: bytes, shares_sold: int, collateral_received: float
) -> bool:
    """
    ...
    try:
        ...
        record_tx = contract.functions.recordProxiedBetSold(
            bet_id, int(shares_sold), int(collateral_received_wei)
        ).build_transaction(
            {
                "nonce": nonce,
                "gasPrice": gas_price,
...
def update_bet_slip_status(
    bet_slip_id: int, pool_configs, execution_result, failure_reason: str
):
    """
    ...
    try:
        ...
        if isinstance(pool_configs, Exception):
            ...
            hasattr(execution_result, "success_rate")
            and execution_result.success_rate > 0
        ):
    ...
    while True:
        ...
        if current_block > last_processed_block:
            try:
                ...
                bet_slip_selling_logs = (
                    contract.events.BetSlipSellingStateUpdate.get_logs(
                        from_block=last_processed_block + 1, to_block=current_block
                    )
                )
